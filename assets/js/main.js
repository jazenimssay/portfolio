...
document.addEventListener("DOMContentLoaded", () => {
  ...
  */
  ...
  if (hireForm) {
    ...
    hireForm.addEventListener("submit", async event => {
      ...
      /*
        With no endpoint there is nowhere to send this,
        so say so rather than pretending it worked.
      */
      if (!endpoint) {
        setResponse(
          "This form isn't connected to an inbox yet — add your form endpoint to send it.",
          "bad"
        );
        return;
      }
  ...
  */
  ...
  function projectCard(project) {
    ...
    return `<button type="button" class="case-file${size}"
  ...
  function renderProjects(list) {
    ...
    if (caseStatus) {
      caseStatus.textContent = shown.length
  ...
  (function initializeProjectViewer() {
    ...
    function openProject(project) {
      ...
      const gallery = Array.isArray(project.gallery) && project.gallery.length
        ? `<div class="project-gallery">` + project.gallery.map(item => `
            <figure>
      ...
      body.innerHTML = `
        ...
        <div class="project-inner">
          ...
          <p class="project-back"><button type="button" class="xp-button" data-close-project>&lt; Back to the drawer</button></p>
    ...
    body.addEventListener("click", event => {
      if (event.target.closest("[data-close-project]")) closeWindow(win);
    });
