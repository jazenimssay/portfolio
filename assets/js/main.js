  ...
  */
  ...
  (function initializeProjectViewer() {
    ...
    function openProject(project, options = {}) {
      ...
      /*
        A gallery is a list of sections. Each carries its own
        column count, so a project can run three across and
        then a strip of seven for a carousel.

        Older flat rows of {src, caption} still work and use
        the project-wide column setting.
      */
      ...
      function sectionMarkup(items, columns, heading) {
        ...
        return `
          ${heading ? `<h3 class="gallery-heading">${escapeHtml(heading)}</h3>` : ""}
          <div class="project-gallery" data-cols="${escapeHtml(String(columns || cols))}">
            ${shots.map(shot => `
              <figure>
                ${mediaTag(shot, project.title)}
      ...
      const rows = Array.isArray(project.gallery) ? project.gallery : [];

      const loose = rows.filter(r => r && r.src && !Array.isArray(r.items));
      const sections = rows.filter(r => r && Array.isArray(r.items));

      const gallery = [
        loose.length ? sectionMarkup(loose, cols, "") : "",
        ...sections.map(s => sectionMarkup(s.items, s.columns, s.heading))
      ].join("");
